...
async function init() {
    ...
    try {
        ...
        // 显示错误信息
        loadingError.textContent = `加载失败: ${error.message}`;
        loadingError.style.display = 'block';
        debugLink.style.display = 'inline-block';

        showToast('加载失败，请查看控制台或点击调试链接', 'error');
    }
}
...
async function loadCreatorData() {
    try {
        // 使用绝对路径从 public 目录加载
        const response = await fetch('/creator-profile/data/creator-data.json');
        if (!response.ok) {
